...
describe('Testing api', function () {
  ...
  it('should retry GET,POST,PUT,DELETE,PATCH,HEAD call on ESOCKETTIMEDOUT', function (done) {
    ...
    setTimeout(function () {
      ...
      client.getAsync('/api/test/error').catch(e1 => {
        expect(e1.message).to.contains('timed out after 5 retries. The connection to the API seems to be overloaded.')
        client.putAsync('/api/test/error').catch(e2 => {
          expect(e2.message).to.contains('timed out after 5 retries. The connection to the API seems to be overloaded.')
          client.postAsync('/api/test/error').catch(e3 => {
            expect(e3.message).to.contains(
              'timed out after 5 retries. The connection to the API seems to be overloaded.'
            )

            client.delAsync('/api/test/error').catch(e4 => {
              expect(e4.message).to.contains(
                'timed out after 5 retries. The connection to the API seems to be overloaded.'
              )

              client.patchAsync('/api/test/error').catch(e5 => {
                expect(e5.message).to.contains(
                  'timed out after 5 retries. The connection to the API seems to be overloaded.'
                )

                client.headAsync('/api/test/error').catch(e6 => {
                  expect(e6.message).to.contains(
                    'timed out after 5 retries. The connection to the API seems to be overloaded.'
                  )
  ...
  it('should retry GET,POST,PUT,DELETE,PATCH,HEAD call 10 times on ESOCKETTIMEDOUT', function (done) {
    ...
    setTimeout(function () {
      const { client } = output.testApi
      client.getAsync('/api/test/error').catch(e1 => {
        expect(e1.message).to.contains('timed out after 10 retries. The connection to the API seems to be overloaded.')

        client.putAsync('/api/test/error').catch(e2 => {
          expect(e2.message).to.contains(
            'timed out after 10 retries. The connection to the API seems to be overloaded.'
          )
  ...
  it('should fail GET,POST,PUT,DELETE,PATCH,HEAD call on ESOCKETTIMEDOUT', function (done) {
    ...
    setTimeout(function () {
      const { client } = output.testApi
      client.getAsync('/api/test/error').catch(e1 => {
        expect(e1.message).to.equal('ESOCKETTIMEDOUT')
        client.putAsync('/api/test/error').catch(e2 => {
          expect(e2.message).to.equal('ESOCKETTIMEDOUT')
          client.postAsync('/api/test/error').catch(e3 => {
            expect(e3.message).to.equal('ESOCKETTIMEDOUT')
            client.delAsync('/api/test/error').catch(e4 => {
              expect(e4.message).to.equal('ESOCKETTIMEDOUT')
              client.patchAsync('/api/test/error').catch(e5 => {
                expect(e5.message).to.equal('ESOCKETTIMEDOUT')
                client.headAsync('/api/test/error').catch(e6 => {
                  expect(e6.message).to.equal('ESOCKETTIMEDOUT')
                  done()
                })
              })
            })
  ...
  it('should fail GET,POST,PUT,DELETE,PATCH,HEAD call on ETIMEDOUT', function (done) {
    ...
    setTimeout(function () {
      const { client } = output.testApi
      client.getAsync('/api/test/error').catch(e1 => {
        expect(e1.message).to.equal('ETIMEDOUT')
        client.putAsync('/api/test/error').catch(e2 => {
          expect(e2.message).to.equal('ETIMEDOUT')
          client.postAsync('/api/test/error').catch(e3 => {
            expect(e3.message).to.equal('ETIMEDOUT')
            client.delAsync('/api/test/error').catch(e4 => {
              expect(e4.message).to.equal('ETIMEDOUT')
              client.patchAsync('/api/test/error').catch(e5 => {
                expect(e5.message).to.equal('ETIMEDOUT')
                client.headAsync('/api/test/error').catch(e6 => {
                  expect(e6.message).to.equal('ETIMEDOUT')
                  done()
                })
              })
            })
